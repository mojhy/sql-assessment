...
def add_animals():
    ...
    if exit_check(animal_name):
        return
    scientific_name = input(f'{bb}scientific name: {reset}')
    if exit_check(scientific_name):
        return
    group_names()
    group = input(f'{bb}group: {reset}')
    if exit_check(group):
        return
    ranking = input(f'{bb}ranking, 81 to 85, 81 is death: {reset}')
    if exit_check(ranking):
        return
